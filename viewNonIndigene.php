...
<div class="pagetitle">
                    <h1>SU Afikpo Zone || All Registered School Leavers</h1>
                    <nav>
                        <ol class="breadcrumb">
        ...
        <table class="table table-bordered">
            ...
            <tbody>
                 <?php
                    ...
                    $sqlTotalItems = "SELECT category, COUNT(*) as nonIndigene FROM campers WHERE category = 'non-indigenes'";
                    $resultTotalItems = mysqli_query($conn, $sqlTotalItems);
                    $rowTotalItems = mysqli_fetch_assoc($resultTotalItems);
                    $totalItems = $rowTotalItems['nonIndigene'];
                    ...
                    $sqlData = "SELECT * FROM campers WHERE category = 'non-indigenes' LIMIT $offset, $itemsPerPage";
                    $resultData = mysqli_query($conn, $sqlData);
                    // display the data
                    $num = 1;
                    if(mysqli_num_rows($resultData) == 0){
                        echo "No Registered camper for this category";
                        die();
                    }
